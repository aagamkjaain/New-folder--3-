...
export default function HotspotScoringTab() {
    ...
    const fetchDeals = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Transform HubSpot deals into hotspot format with time saved scoring
          const hubspotHotspots = (data || []).map((deal: any) => {
            const dealValue = parseInt(deal.dealstage?.value?.split(',')[0]) || 0;
            const timeSavedHours = Math.round((dealValue / 100000) * 20); // Estimate time saved per deal
            const daysSaved = Math.ceil(timeSavedHours / 8);
            const costOfDelay = dealValue > 100000 ? 5000 : 2500; // Higher CoD for bigger deals
            
            // Calculate hotspot score based on deal value and stage
            const stageWeights: Record<string, number> = {
              'contractsent': 95,
              'presentationscheduled': 85,
              'negotiation': 80,
              'decisionmakersengaged': 75,
              'qualifiedtobuy': 60,
              'appointmentscheduled': 50,
            };
            const hotspotScore = stageWeights[deal.dealstage?.value?.toLowerCase() || ''] || 50;
            ...
            return {
              ...
              dealValue: dealValue,
              dueDate: deal.closeddate?.value || 'TBD',
              status: dealValue > 100000 ? 'Critical' : dealValue > 50000 ? 'At Risk' : 'Moderate',
              hotspotScore,
              slack: daysSaved > 0 ? `+${daysSaved} days` : `-${Math.abs(daysSaved)} days`,
              cod: `$${costOfDelay.toLocaleString()}/day`,
              proximity: `${daysSaved + 5} days`,
              dependencies: `${Math.floor(dealValue / 25000)} tasks`,
              impact: `AI-powered scoring identified ${timeSavedHours}h of time savings through deal automation. Estimated deal value: $${dealValue.toLocaleString()}. Time saved could be redeployed to close similar-sized deals.`,
              borderColor: dealValue > 100000 ? 'border-red-400' : dealValue > 50000 ? 'border-yellow-400' : 'border-blue-400',
              timeSavedHours,
              daysSaved,
            };
          });
          setDeals(hubspotHotspots.sort((a, b) => b.dealValue - a.dealValue));
        }
      } catch (err) {
  ...
  const hotspots = deals.length > 0 ? deals : [
    {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Revenue Deals & Task Hotspots</h2>
        <p className="text-xs sm:text-sm text-gray-600 mt-2">
          {deals.length > 0 ? 'HubSpot deal pipeline scored by time savings & cost of delay' : 'Ranked by slack, proximity, and Cost of Delay (CoD)'}
        </p>
      </div>
