...
export default function DashboardTab() {
  ...
  // Load HubSpot deals to compute the same returns as VPDashboard
  useEffect(() => {
    let mounted = true;
  ...
  // Compute total returns using same assumptions as VPDashboard
  ...
  const hubspotReturns = hubspotCostSavingUSD + hubspotRevenueAcceleration;
  // Other tools fixed returns to mirror VPDashboard
  const otherReturns = 500 + 395 + 115 + 77;
  const displayedTotalReturnsUSD = Math.round(hubspotReturns + otherReturns);

  const TOTAL_INVESTMENT_USD = 50000; // match VPDashboard assumption
  const blendedROI = totalReturns != null ? ((totalReturns - TOTAL_INVESTMENT_USD) / TOTAL_INVESTMENT_USD) * 100 : null;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 hover:shadow-lg transition-shadow">
          ...
          <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-green-600">{formatLargeUSD(displayedTotalReturnsUSD || totalReturns)}</div>
