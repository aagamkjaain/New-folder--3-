...
export default function RedeploymentTab() {
    ...
    const fetchDeals = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setDeals((data || []).sort((a: any, b: any) => (parseInt(b.dealstage?.value) || 0) - (parseInt(a.dealstage?.value) || 0)));
        }
      } catch (err) {
  ...
  // Calculate HubSpot revenue impact
  const topDeal = deals[0];
  const topDealValue = topDeal ? parseInt(topDeal.dealstage?.value || '0') : 150000;
  const revenueOpportunity = Math.round((allocatedHours / 160) * topDealValue); // Based on monthly capacity
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Capacity Redeployment & Revenue Impact</h2>
        <p className="text-xs sm:text-sm text-gray-600 mt-1">Allocate freed time to revenue-critical deals • Predict acceleration & revenue impact</p>
      </div>

      {loading && (
        <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
          <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
          <p className="text-gray-600 mt-2">Loading HubSpot deals...</p>
        </div>
      )}
      ...
      {!loading && (
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6">
          ...
          <div className="p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div className="flex justify-between pt-1.5 sm:pt-2 border-t border-blue-200">
                ...
                <span className="font-bold text-green-600">${revenueOpportunity.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">ROI Tier:</span>
                <span className="px-2 py-1 bg-green-100 text-green-700 text-xs font-bold rounded">Tier A - Revenue</span>
              </div>
            </div>
        ...
        {/* Active Allocations - HubSpot Deals */}
        <div className="mt-4 sm:mt-6 bg-white rounded-lg border border-gray-200 p-4 sm:p-6">
        <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-4">HubSpot Revenue Deals - Capacity Allocation</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-3 sm:px-6 py-2 sm:py-3 text-left font-bold text-gray-700 uppercase">Deal Name</th>
                <th className="px-3 sm:px-6 py-2 sm:py-3 text-left font-bold text-gray-700 uppercase">Deal Value</th>
                <th className="px-3 sm:px-6 py-2 sm:py-3 text-left font-bold text-gray-700 uppercase">Allocated Hours</th>
                <th className="px-3 sm:px-6 py-2 sm:py-3 text-left font-bold text-gray-700 uppercase">Time Saved Impact</th>
              </tr>
            </thead>
            <tbody>
              {deals.slice(0, 3).map((deal, idx) => {
                const dealValue = parseInt(deal.dealstage?.value || '0');
                const allocatedByDeal = Math.floor((allocatedHours / deals.length) * 3);
                const daysSavedByDeal = Math.ceil(allocatedByDeal / 8);
                return (
                  <tr key={idx} className="border-b border-gray-100">
                    <td className="px-3 sm:px-6 py-2 sm:py-4 font-semibold text-gray-900">{deal.dealname || 'Unnamed'}</td>
                    <td className="px-3 sm:px-6 py-2 sm:py-4 font-bold text-blue-600">${dealValue.toLocaleString()}</td>
                    <td className="px-3 sm:px-6 py-2 sm:py-4 text-gray-600">{allocatedByDeal}h</td>
                    <td className="px-3 sm:px-6 py-2 sm:py-4 font-bold text-green-600">
                      {daysSavedByDeal} day{daysSavedByDeal > 1 ? 's' : ''} acceleration
                    </td>
                  </tr>
                );
              })}
