...
const ROIReport = () => {
  ...
  const [hubspotPipelineValue, setHubspotPipelineValue] = useState(0);
  const [showFormModal, setShowFormModal] = useState(false);
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", company: "", message: "" });
  const [formSubmitted, setFormSubmitted] = useState(false);

  useEffect(() => {
    // Fetch HubSpot deals to calculate pipeline value
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="mb-8 bg-gradient-hero p-8 text-center text-white shadow-2xl md:p-12">
          ...
          <div className="mb-2 text-sm text-white/80">AI Time Savings + HubSpot Revenue Opportunity</div>
            <div className="mb-4 text-5xl font-bold text-[#77ff00] md:text-6xl">
            ${(yearlyROI + (hubspotPipelineValue * 0.05)).toLocaleString()}
          </div>
          <div className="mb-6 text-xl font-semibold" style={{ color: "#77ff00" }}>
            Forecasted Annual Value (Savings + Revenue)
          </div>
          <p className="mb-2 text-lg">
            <strong>
              <span style={{ color: "#77ff00" }}>${roi.toLocaleString()}</span>
              {' '}
              from AI-freed time ({totalHours} hours){hubspotPipelineValue > 0 && (
                <>
                  {' '}+ <span style={{ color: "#77ff00" }}>${(hubspotPipelineValue * 0.05).toLocaleString()}</span> HubSpot pipeline acceleration
                </>
              )}
            </strong>
          </p>
